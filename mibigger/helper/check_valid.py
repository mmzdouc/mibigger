...
with open(Path.cwd().joinpath("mibigger", "helper", "schema.json")) as schema_handle:
...
def check_pks_module_duplication(data: Dict[str, Any], prefix: str) -> bool:
    for synthase in data["cluster"].get("polyketide", {}).get("synthases", []):
        ...
        if duplicates:
            print(
                f"{prefix}contains duplicate PKS module numbers: {sorted(duplicates)}"
            )
            return False
    return True
...
def check_nrp_opts(data: Dict[str, Any], prefix: str) -> bool:
    if "nrp" in data["cluster"] and "NRP" not in data["cluster"]["biosyn_class"]:
        print(
            f"{prefix}contains nrp details but doesn't list 'NRP' as biosynthetic class"
        )
        return False
    return True
